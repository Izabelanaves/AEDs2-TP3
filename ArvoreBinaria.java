...
class Arvore {
	...
	private No inserirRec(No no, Restaurante r) {
		if (no == null) return new No(r);
		if (r.getNome().compareTo(no.restaurante.getNome()) < 0)
			no.esq = inserirRec(no.esq, r);
		else
			no.dir = inserirRec(no.dir, r);
		return no;
	}

	String pesquisar(String nome) {
		return pesquisarRec(raiz, nome, "raiz");
	}
