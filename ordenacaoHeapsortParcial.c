...
static int comparaData(Data a, Data b)
{
	if (a.ano != b.ano)
		return a.ano - b.ano;
	if (a.mes != b.mes)
...
{
	int cmp = comparaData(a.dataAbertura, b.dataAbertura);
	if (cmp != 0)
		return cmp;
...
static void heapify(ColecaoRestaurantes *c, int n, int i)
{
	int maior = i;
	...
	if (esq < n) {
		++num_comparacoes;
		if (compara(c->restaurantes[esq], c->restaurantes[maior]) > 0)
			maior = esq;
	}
	if (dir < n) {
		++num_comparacoes;
		if (compara(c->restaurantes[dir], c->restaurantes[maior]) > 0)
			maior = dir;
	}

	if (maior != i) {
		swap(&c->restaurantes[i], &c->restaurantes[maior]);
		heapify(c, n, maior);
	}
}

static void heapsortParcial(ColecaoRestaurantes *c, int k)
{
	int n = c->tamanho;
	if (k > n)
		k = n;
	...
	for (int i = k; i < n; ++i) {
		++num_comparacoes;
		if (compara(c->restaurantes[i], c->restaurantes[0]) < 0) {
			swap(&c->restaurantes[0], &c->restaurantes[i]);
			heapify(c, k, 0);
		}
	}
...
int main(void)
{
	...
	FILE *log_file = fopen("859563_heapsort_parcial.txt", "w");
