...
static bool pesquisaBinaria(ColecaoRestaurantes *c, char *nome)
{
	...
	while (esq <= dir && !encontrado) {
		int meio = (esq + dir) / 2;
		++num_comparacoes; // Conta comparacao
		int cmp = strcmp(c->restaurantes[meio].nome, nome);
		if (cmp == 0)
			encontrado = true;
		else if (cmp < 0)
			esq = meio + 1;
...
int main(void)
{
	...
	char nomesBusca[MAX_RESTAURANTES][MAX_NOME];
	int nBusca = 0;
	while (scanf(" %[^\r\n]", nomesBusca[nBusca]) == 1) {
		if (strcmp(nomesBusca[nBusca], "FIM") == 0)
			break;
		nBusca++;
	}
	...
	selecao(selecionados); // Ordena para viabilizar busca binaria
	for (int i = 0; i < nBusca; i++)
		puts(pesquisaBinaria(selecionados, nomesBusca[i]) ? "SIM" : "NAO");
	clock_gettime(CLOCK_MONOTONIC, &time_end);
	double duracao_segundos = time_end.tv_sec - time_begin.tv_sec + ((double)(time_end.tv_nsec - time_begin.tv_sec))/1000000000;

	// Log
	FILE *log_file = fopen("859563_binaria.txt", "w");
