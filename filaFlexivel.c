...
Fila *novaFila(void)
{
	...
	f->primeiro = calloc(1, sizeof(Celula));
	f->ultimo = f->primeiro;
	f->tamanho = 0;
	return f;
}
...
{
	f->ultimo->prox = calloc(1, sizeof(Celula));
	f->ultimo = f->ultimo->prox;
	f->ultimo->restaurante = r;
	f->ultimo->prox = NULL;
	f->tamanho++;
}
...
int main(void)
{
	...
	FILE *log_file = fopen("859563_filaFlexivel.txt", "w");
