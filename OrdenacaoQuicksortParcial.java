...
public class OrdenacaoQuicksortParcial {
	...
	public static void main(String[] args) throws IOException
	{
		...
		quicksortParcial(arrayBusca, 0, n - 1, 10);
		double tempoExecucaoMs = (System.nanoTime() - inicio) / 1_000_000.0;

		try (FileWriter writer = new FileWriter("859563_quicksort_parcial.txt")) {
	...
	static void swap(Restaurante[] vec, int i, int j)
	{
		Restaurante tmp = vec[i];
		vec[i] = vec[j];
		vec[j] = tmp;
		++OrdenacaoQuicksortParcial.movimentacoes;
	}

	static void quicksortParcial(Restaurante[] vec, int esq, int dir, int k)
	{
		if (esq >= dir || esq >= k)
			return;

		int i = esq, j = dir;
		Restaurante pivo = vec[(esq + dir) / 2];

		while (i <= j) {
			while (++OrdenacaoQuicksortParcial.comparacoes > 0 && compara(vec[i], pivo) < 0)
				i++;
			while (++OrdenacaoQuicksortParcial.comparacoes > 0 && compara(vec[j], pivo) > 0)
				j--;
			if (i <= j) {
				swap(vec, i, j);
				i++;
		...
		quicksortParcial(vec, esq, j, k);
		if (i < k)
			quicksortParcial(vec, i, dir, k);
	}
}
