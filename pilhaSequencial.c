...
int main(void)
{
	...
	ColecaoRestaurantes *pilha = calloc(1, sizeof(*pilha));
	lerCsv(colecao, "/tmp/restaurantes.csv");

	int idBuscado;
	char buf[2048];

	while (scanf("%d", &idBuscado) == 1 && idBuscado != -1) {
		for (int i = 0; i < colecao->tamanho; i++) {
			if (colecao->restaurantes[i].id == idBuscado) {
				pilha->restaurantes[pilha->tamanho++] = colecao->restaurantes[i];
				i = MAX_RESTAURANTES; 
			}
		}
	}
	...
	if (scanf("%d", &qtdComandos) == 1) {
		for (int k = 0; k < qtdComandos; k++) {
			...
			if (strcmp(comando, "I") == 0) {
				...
				if (pilha->tamanho > 0) {
					pilha->tamanho--;
					printf("(R)%s\n", pilha->restaurantes[pilha->tamanho].nome);
				}
			}
		}
	}
	for (int i = pilha->tamanho - 1; i >= 0; i--)
		puts(formatarRestaurante(&pilha->restaurantes[i], buf));
