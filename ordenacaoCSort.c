...
static void parseTiposCozinha( char *s, char tipos[][MAX_NOME], int *numTipos)
{
	*numTipos = 0;
	char temp[MAX_LINHA];
...
int getMaiorCapacidade(ColecaoRestaurantes *c) {
	int maior = c->restaurantes[0].capacidade;
	for (int i = 1; i < c->tamanho; i++) {
		num_comparacoes++; 
		if (c->restaurantes[i].capacidade > maior) {
			maior = c->restaurantes[i].capacidade;
		}
	}
	return maior;
}

void countingsort(ColecaoRestaurantes *c) {
	int n = c->tamanho;
	if (n <= 1) return;

	int max = getMaiorCapacidade(c);
	int *count = calloc(max + 1, sizeof(int));

	Restaurante *output = malloc(n * sizeof(Restaurante));
	for (int i = 0; i < n; i++) {
		count[c->restaurantes[i].capacidade]++;
	}
	for (int i = 1; i <= max; i++) {
		count[i] += count[i - 1];
	}
	for (int i = n - 1; i >= 0; i--) {
		int cap = c->restaurantes[i].capacidade;
		output[count[cap] - 1] = c->restaurantes[i];
		count[cap]--;
		num_movimentacoes++;
}
	for (int i = 0; i < n; i++) {
		c->restaurantes[i] = output[i];
		num_movimentacoes++;
	}
