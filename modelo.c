...
static void parseTiposCozinha(const char *s, char tipos[][MAX_NOME], int *numTipos)
{
	*numTipos = 0;
	...
	while (token != NULL && *numTipos < MAX_TIPOS) {
		strncpy(tipos[*numTipos], token, MAX_NOME - 1);
...
Restaurante parseRestaurante(const char *s)
{
	...
	strncpy(linha, s, sizeof(linha) - 1);
	...
	while (token != NULL) {
		...
		case 0:
			...
			break;
		case 1:
			strncpy(r.nome, token, MAX_NOME - 1);
			r.nome[MAX_NOME - 1] = '\0';
			break;
		case 2:
			strncpy(r.cidade, token, MAX_CIDADE - 1);
		...
		case 4:
			...
			break;
		case 5:
			strncpy(tiposStr, token, sizeof(tiposStr) - 1);
...
char *formatarRestaurante(const Restaurante *r, char *buf)
{
	...
	for (int i = 0; i < r->numTipos; i++) {
		strcat(tiposFormatados, r->tiposCozinha[i]);
		if (i < r->numTipos - 1)
			strcat(tiposFormatados, ",");
	}
	...
	while (scanf("%d", &idBuscado) == 1) {
		for (int i = 0; i < colecao->tamanho; i++) {
			if (colecao->restaurantes[i].id == idBuscado) {
				printf("%s\n", formatarRestaurante(&colecao->restaurantes[i], buf));
				i = MAX_RESTAURANTES;
			}
		}
	}
